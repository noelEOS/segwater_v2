...
OUT = Path.home()/"demak_full_3lineage_water_area_timeseries.csv"
...
ARMS = ["mx630s2_best", "mx630s2_last", "mx630k"]
...
def main():
    rows, seen = [], {}
    for arm in ARMS:
        hits = [d for d in sorted(RUNS.glob("demak_full_%s_*" % arm)) if d.is_dir()]
    ...
    print("\nwrote %s (%d rows = 3 arms x 213)" % (OUT, len(df)))
    print("in analysis window: %d per arm" % df[df.arm == "mx630k"].in_analysis_window.sum())
