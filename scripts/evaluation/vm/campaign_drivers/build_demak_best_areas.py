...
OUT = Path.home()/"demak_full_3seed_BEST_water_area_timeseries.csv"
...
def audit(base, seed):
    ...
    # BEST arm: checkpoint must be a pmwiou-selected one, NOT the final step.
    assert "_pmwiou" in ck, "not a pmwiou (best) ckpt: %s" % ck
    assert not ck.endswith("_last.pth"), "this is the LAST ckpt, not best: %s" % ck
