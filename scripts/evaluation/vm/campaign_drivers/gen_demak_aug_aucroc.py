...
evaluation:
  ...
  model_runs:
    Upernet_Swin_Base_224_s42aug_{arm}_native224_weighted:
      run_dir: "{run_dir}"

  missing_prediction_policy: "record_and_continue"

output:
  root: "/home/noel/demak_gate_aug_eval"
  run_name: "{name}"
  add_timestamp: false
"""
...
for arm in ["best", "last"]:
    hits = sorted(d for d in RUNS.glob("demak_s42aug_%s_*" % arm) if d.is_dir())
    if len(hits) != 1:
