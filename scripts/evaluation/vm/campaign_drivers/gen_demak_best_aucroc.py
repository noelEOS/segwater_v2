...
OUT  = Path.home()/"configs/noaug_best_3seed"
TMPL = """\
...
evaluation:
  ...
  model_runs:
    Upernet_Swin_Base_224_{seed}_noaug_best_native224_weighted:
      run_dir: "{run_dir}"
  missing_prediction_policy: "record_and_continue"
output:
  root: "/home/noel/demak_best_3seed_eval"
  run_name: "{name}"
  add_timestamp: false
"""
prob=[]
for seed in ["s19","s42","s58"]:
    pats=[f"demak_{seed}noaug_best_*", f"demak_{seed}noaug_best_*"]
    hits=[d for d in sorted(RUNS.glob(f"demak_{seed}noaug_best_*")) if d.is_dir()]
    ...
    name=f"demak_{seed}noaug_best_s32"
