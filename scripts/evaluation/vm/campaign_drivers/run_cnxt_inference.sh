#!/bin/bash
# ConvNeXtV2-Tiny mx630_stage2 SHIP decision — all inference, SERIAL (one GPU).
...
cd ~/segwater_v2
C=~/configs/ship_decision_cnxt_2026-07
mkdir -p ~/workspace/logs/cnxt
run_phase () {
  local phase="$1"; shift
  echo "=== PHASE $phase START $(date -u +%H:%M:%S) ==="
  for cfg in "$@"; do
    n=$(basename "$cfg" .yaml); s=$(date +%s)
    if python scripts/run_inference_sweep.py "$cfg" --strict > ~/workspace/logs/cnxt/$n.log 2>&1; then
      e=$(date +%s)
      echo "  DONE $n ($((e-s))s) $(grep -oE 'Successful scenes: [0-9]+ \| Failed scenes: [0-9]+' ~/workspace/logs/cnxt/$n.log | tail -1)"
    else
      e=$(date +%s); echo "  FAILED $n ($((e-s))s)"; tail -5 ~/workspace/logs/cnxt/$n.log
    fi
  done
  echo "=== PHASE $phase COMPLETE $(date -u +%H:%M:%S) ==="
}
run_phase 1-demak_gate $C/demak_gate/*.yaml
...
echo "=== ALL CNXT INFERENCE FINISHED $(date -u +%H:%M:%S) ==="
