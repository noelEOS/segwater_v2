...
OK=0; FAIL=0
for rp in ~/segwater_v2/outputs/inference/runs/narrabeen_s19noaug_last_* ~/segwater_v2/outputs/inference/runs/narrabeen_s58noaug_last_*; do
  [ -d "$rp" ] || continue
  n=$(basename "$rp")
  echo "=== SDS START $n ==="
  if python scripts/sds/run_sds_from_rasters.py --site NARRABEEN --raster-dir "$rp" \
       --out-dir ~/sds_vm_eval/"${n}_sweep" --thresholds "$THR" \
...
echo "=== NARRABEEN 2SEED SDS FINISHED: $OK ok, $FAIL failed ==="
