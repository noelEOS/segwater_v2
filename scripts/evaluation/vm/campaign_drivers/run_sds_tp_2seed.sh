...
OK=0; FAIL=0
for rp in ~/segwater_v2/outputs/inference/runs/torreypines_s19noaug_last_* ~/segwater_v2/outputs/inference/runs/torreypines_s58noaug_last_*; do
  [ -d "$rp" ] || continue
  n=$(basename "$rp"); echo "=== SDS START $n ==="
  if python scripts/sds/run_sds_from_rasters.py --site TORREYPINES --raster-dir "$rp" \
       --out-dir ~/sds_vm_eval_torreypines/"${n}_sweep" --thresholds "$THR" --no-min-chainage-length \
...
echo "=== TORREYPINES 2SEED SDS FINISHED: $OK ok, $FAIL failed ==="
