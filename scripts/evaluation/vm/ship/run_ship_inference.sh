#!/bin/bash
# mx630_stage2 SHIP decision — all inference, SERIAL (one GPU).
...
run_phase () {
  ...
  for cfg in "$@"; do
    ...
    if python scripts/run_inference_sweep.py "$cfg" > ~/workspace/logs/ship/$n.log 2>&1; then
      ...
      e=$(date +%s); echo "  FAILED $n ($((e-s))s)"; tail -5 ~/workspace/logs/ship/$n.log
    fi
  done
...
echo "=== ALL SHIP INFERENCE FINISHED $(date -u +%H:%M:%S) ==="
