...
Wraps a frozen HuggingFace DINOv3ViTModel with a single 1×1 conv head and
bilinear ×16 upsample. The interface mirrors smp models: forward() accepts a
raw pixel tensor (B, C, H, W) with z-score normalised values and returns
...
The backbone is always frozen after adaptation; only the linear head is trained.
This matches the "linear probe" regime described in the DINOv3 model card.
"""
...
class DINOv3LinearSegmentation(nn.Module):
    """Frozen DINOv3 ViT backbone + trainable linear segmentation head.
    ...
    def train(self, mode: bool = True):
        """Keep backbone in eval mode even when the module is set to train."""
        super().train(mode)
        self.backbone.eval()
        return self
